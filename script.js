...
const FILE_PREFIX =
  "https://magentadata.storage.googleapis.com/piano_transformer/midi/";
...
const player = new core.SoundFontPlayer(
  "https://storage.googleapis.com/download.magenta.tensorflow.org/soundfonts_js/salamander"
);
...
function init() {
  ...
  document
    .getElementById("btnFave")
    .addEventListener("click", faveOrUnfaveSong);
  document
    .getElementById("btnPlaylist")
...
function save() {
  ...
  window.saveAs(
    new File([window.core.sequenceProtoToMidi(song.sequence)], song.fileName)
  );
}
...
function changeSong(index, noAutoplay = false) {
  ...
  const twitterPrefix =
    "https://twitter.com/intent/tweet?hashtags=madewithmagenta&text=Listen%20to%20this%20Piano%20Transformer%20composition%21%20";
...
function updateFaveButton() {
  ...
  const index = faves.findIndex(
    (x) => x.name === allData[currentSongIndex].fileName
  );
